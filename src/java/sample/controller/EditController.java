    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (session != null) {
                ...
                if (cart != null) {
                    FlowerDTO flower = cart.getCart().get(productID);
                    FlowerDAO flowerDao = new FlowerDAO();
                    FlowerDTO flowerCart = flowerDao.getFlowerID(productID);
                    if (quantity > flowerCart.getQuantity()) {
                        request.setAttribute("MESSAGE", "Only " + flowerCart.getQuantity() + " product left in stock");
                    } else {
                        flower.setQuantity(quantity);
                        boolean check = cart.edit(productID, flower);
                        if (check) {
